...
public class SoldScripFragment extends Fragment {
    ...
    private TextView tvTotAmtInv, tvTotSoldValue, tvTotGain, tvTotAbs;
    private double mTotAmtInv, mTotSoldValue, mTotGain, mTotAbs;
    ...
    public static SoldScripFragment newInstance() {

        Bundle args = new Bundle();

        SoldScripFragment fragment = new SoldScripFragment();
        fragment.setArguments(args);
        return fragment;
    }

    public SoldScripFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_sold_scrip, container, false);
        findViews(rootView);
        setViews();
    ...
    private void insertHoldings(final ResSoldScripDetails resSoldScripDetails) {
        ...
        realm.executeTransactionAsync(new Realm.Transaction() {
            ...
            public void execute(Realm realm) {
                Log.d(LOG_TAG,
                        C.net.tag.DB_TRANSACTION_WRITE + "DbSoldScripDetails");
                realm.where(DbSoldScripDetails.class).findAll().deleteAllFromRealm();
                realm.where(DbLogs.class).findAll().deleteAllFromRealm();
                for (int i = 0; i < resListSoldScripEntities.size(); i++) {

                    DbSoldScripDetails dbSoldScripDetails = new DbSoldScripDetails(resListSoldScripEntities.get(i));

                    realm.copyToRealmOrUpdate(dbSoldScripDetails);
                }

                try {
                    /*for (int i = 0; i < resListSoldScripEntities.size(); i++) {
                        double valueAmtInv = Double.valueOf(resListSoldScripEntities.get(i).getAmtInvested());
                        double valueMktValue = Double.valueOf(resListSoldScripEntities.get(i).getMarketValue());
                        double valueAbs = Double.valueOf(resListSoldScripEntities.get(i).getAbsoluteReturn());
                        double valueGain = Double.valueOf(resListSoldScripEntities.get(i).getGain());

                        mTotAmtInv = mTotAmtInv + valueAmtInv;
                        mTotSoldValue = mTotSoldValue + valueMktValue;
                    ...
                    Log.v(LOG_TAG, C.net.tag.DB_TRANSACTION_WRITE + "Already DbSoldScripDetails present ", e);
                } finally {
    ...
    private void findViews(View rootView) {
        mRecyclerView = (RecyclerView) rootView.findViewById(R.id.rvSoldScrip);
        ...
        tvTotSoldValue = (TextView) rootView.findViewById(R.id.tvTotSoldValue);
    ...
    private void setUpRecyclerView() {
        ...
        if (mDbSoldScripDetails.isEmpty()) {
            ...
            tvTotAbs.setText(realm.where(DbSoldScripDetails.class).sum("absoluteReturn").toString());
            tvTotAmtInv.setText(realm.where(DbSoldScripDetails.class).sum("amtInvested").toString());
            tvTotGain.setText(realm.where(DbSoldScripDetails.class).sum("gain").toString());
            tvTotSoldValue.setText(realm.where(DbSoldScripDetails.class).sum("marketValue").toString());
    ...
    private class CallHolder extends RecyclerView.ViewHolder {
        ...
        public void bindFeed(DbSoldScripDetails dbSoldScripDetails) {
            ...
            tvWt.setText(this.dbSoldScripDetails.getWeightage());
            tvIndustry.setText(this.dbSoldScripDetails.getIndustry());
            double value = Double.parseDouble(this.dbSoldScripDetails.getAbsoluteReturn()+"");
            if (value < 0) {
            ...
            value = Double.parseDouble(this.dbSoldScripDetails.getGain()+"");
            if (value < 0) {
